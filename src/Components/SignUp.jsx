...
function SignUp(){
    ...
    const {Signin,name}=React.useContext(AppContext);
    ...
    <Box mt="30px" w="25%" m="auto">
        <Text fontSize="28px" fontWeight="bold" textAlign="left" mt="20px">Create an account</Text>
        <Box sx={{border:"1px solid black",borderRadius:"10px"}} mt="20px">
            <Input ref={refData} name="email" border="none" type="email" onChange={handleChange} placeholder="Email address"/>
        </Box>
        <Box sx={{border:"1px solid black",borderRadius:"10px"}} mt="20px">
            <Input ref={refData} name="firstName" border="none" type="text" onChange={handleChange} placeholder="First name"/>
        </Box>
        <Box sx={{border:"1px solid black",borderRadius:"10px"}} mt="20px">
            <Input ref={refData} name="lastName" border="none" type="text" onChange={handleChange} placeholder="Last name"/>
        </Box>
        <Flex sx={{border:"1px solid black",borderRadius:"10px"}} mt="15px" alignItems="center">
            <Input ref={refData} name="password" border="none" type="password" onChange={handleChange} placeholder="Password" />
        ...
        <Button mt="30px" bg="blue.600" color="white" w="99%" h="45px" onClick={()=>Signin(data.email,data.password,data.firstName)}>continue</Button>
        <Text mt="30px"fontSize="14px" color="blue">Forgot password?</Text>
        <Flex m="auto" w="60%" alignItems="center"><Text mt="30px" fontSize="14px">Already have an account?</Text><Link to="/login"><Text fontSize="14px" mt="30px" ml="15px" color="blue"> Sign in</Text></Link></Flex>
